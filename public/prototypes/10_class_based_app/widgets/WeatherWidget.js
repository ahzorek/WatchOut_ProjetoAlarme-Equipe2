...
class WeatherWidget {
  ...
  createStructure() {
    ...
    this.widgetElement.classList.add('weather-card', 'animate')
    this.container.appendChild(this.widgetElement)

    this.updateWeatherData(this.app.state.weather)

  }

  updateWeatherData({ cityName, temperature, minTemperature, maxTemperature, weatherIconURL, isLoading, error }) {
    this.unit = this.app.state.user.unit
